...
PLAN_SYSTEM_PROMPT = (
    ...
    "when you provide x/y or a bbox, those coordinates are the click position "
    "and will be used directly; do not rely on a different icon or OCR match "
    "to override them; "
    ...
    "set requires_smart_model=true only when the task genuinely needs complex "
        ...
        "mode and is slower, so use this flag sparingly. Resolve visual uncertainty "
        "yourself from the attached screenshot, OCR coordinates, recognised icons, "
        "or the strongest safe visual anchor; never ask the user which icon, button, "
        "contact, or screen target to press. Use action=ask_user only when the user's "
        "intent is genuinely ambiguous (for example, two different files or apps "
        "were requested and the instruction gives no preference). For that rare case, "
        "emit one action=ask_user step with params.question and params.options "
        "(an array of 2-6 concise choices), then re-plan from the answer.\n"
